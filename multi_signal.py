...
from resco_benchmark.junction_manager import JunctionManager
...
class MultiSignal(gym.Env):
    def __init__(self, run_name, map_name, net, state_fn, reward_fn, route=None, gui=False, end_time=3600,
        ...
        if self.route is not None:
            ...
            sumo_cmd = [sumolib.checkBinary('sumo'), '-n', net, '-r', self.route + '_1.rou.xml', '--no-warnings',
                        'True']
        else:
        ...
        self.junction_manager = JunctionManager(self.sumo, self.map_name)
        if not self.libsumo: traci.switch(self.connection_name)
        traci.close()
        self.connection_name = run_name + '-' + map_name + '-' + str(
            len(lights)) + '-' + state_fn.__name__ + '-' + reward_fn.__name__
    ...
    @property
    def junction_manager(self):
        return self._junction_manager
    ...
    @junction_manager.setter
    def junction_manager(self, value):
        self._junction_manager = value
